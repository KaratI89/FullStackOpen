...
export const App = () => {
  ...
  const addPerson = (event) => {
    ...
    console.log(numb);
    if (numb === -1 && !newName == "" && !newNumber == "") { //=== определяет любые строки, не зависимо от содержания(не работает). == работает
      phonebookService
  ...
  return (
    <div>
      ...
      <h2>Numbers</h2>
      <ul> 
        {filteredPeople.map((person) => <Person key={person.name} person={person} deleteFunction={deletePerson} changeFunction={()=>change(person.id)} />)} 
      </ul>
    </div>
  )}
//это массив из Person каждый элемент хранит свои данные и ссылку на функцию.
export default App